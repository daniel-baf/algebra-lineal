...
public class ParserController<T> {
    ...
    public void saveVector(ArrayList<GraphVector> vectors) {
        vectors.forEach(vector -> {
            boolean valueNonExist = Objects.isNull(this.model.getVectors().get(vector.getName()));
            if (valueNonExist) {
                this.model.getVectors().put(vector.getName(), vector);
            }
            CustomLogger.getInstance().addLog(String.format("AVOIDING ADDITION OF VECTOR WITH NAME %1$s, it already exists ", vector.getName()), !valueNonExist);
        });
    }
