...
public class GaussJordanSolver {
    ...
    public Matrix solveGauss(Matrix matrix, boolean verbose) {
        ...
        try {
            ...
            CustomLogger.getInstance().addLog("RESULTADO GAUSS: " + tmpMatrix.toString(), verbose);
            solveSubstitution(tmpMatrix, verbose);
            tmpMatrix.setName(String.format("GAUSS(%1$s)", tmpMatrix.getName()));
            return tmpMatrix;
    ...
    public Matrix solveGaussJordan(Matrix matrix) {
        return this.solveGaussJordan(matrix, false);
    }
