...
//            main.testDecrypt(tmpMatrix, true);
    ...
    private Matrix testEncrypt(boolean verbose) {
        double[][] key = {{1, -3, 2}, {0, - 4, 3}, {1, 4, 21}};
        Crypter encrypter = new Crypter(new Matrix("KEY", key));
        Matrix encrypMatrix = encrypter.encrypt("PATO POLLO", verbose);
    ...
    private void testDecrypt(Matrix matrix, boolean verbose) {
        double[][] key = {{1, -3, 2}, {0, - 4, 3}, {1, 4, 21}};
        Crypter encrypter = new Crypter(new Matrix("KEY", key));
        matrix.setName("Encrypted");
        // code to decrypt testing
        String decryptedText = encrypter.decrypt(matrix, verbose);
